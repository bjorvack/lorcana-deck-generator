...
(async () => {
  ...
  try {
    ...
    while (hasMorePages) {
      ...
      await new Promise(resolve => setTimeout(resolve, 3000 + Math.random() * 2000)) // 3-5s random delay
      ...
      for (const tournament of tournamentLinks) {
        ...
        await new Promise(resolve => setTimeout(resolve, 2000 + Math.random() * 2000)) // 2-4s random delay
          ...
          if (exportUrlMatch) {
            ...
            while (retryCount < maxRetries && !cardList) {
              ...
              try {
                ...
                await new Promise(resolve => setTimeout(resolve, 4000 + Math.random() * 2000)) // 4-6s random wait for Cloudflare
